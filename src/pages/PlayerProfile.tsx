    ...
    const fetchData = async () => {
      ...
      if (PlayerId) {
        ...
        if (!profileData) {
          profileData = {
            id: PlayerId,
            user_id: PlayerId === '10027189' ? 'demo-user-1' : `user-${PlayerId}`,
            IdNumber: PlayerId,
            full_name: PlayerId === '10027189' ? 'demo' : `Athlete ${PlayerId}`,
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-12 flex flex-col gap-6 sm:gap-8">
      ...
      <div className="flex flex-col items-center text-center gap-4 bg-white border border-gray-200 rounded-3xl p-6 sm:p-8 shadow-sm relative overflow-hidden">
        ...
        {/* Social Links & Verified Instagram Button */}
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mt-2 w-full">
          {profile.ig_link && (
            <a 
              href={profile.ig_link.startsWith('http') ? profile.ig_link : `https://instagram.com/${profile.ig_link.replace('@', '')}`}
              target="_blank" 
              ...
              title="Visit Instagram to verify legitimacy"
            >
          ...
          {profile.twitter_link && (
            <a 
              href={profile.twitter_link.startsWith('http') ? profile.twitter_link : `https://${profile.twitter_link}`}
              target="_blank" 
              rel="noopener noreferrer"
